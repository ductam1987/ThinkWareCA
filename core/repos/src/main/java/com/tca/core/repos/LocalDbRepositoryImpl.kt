...
import com.tca.core.model.db.DbMovieDetail_
import com.tca.core.model.db.DbMovies
import com.tca.core.model.db.MyObjectBox
...
class LocalDbRepositoryImpl : LocalDbRepository {
    ...
    override fun getListMovie(recommendConfigDb: RecommendConfigDb): List<DbMovie> =
        movieBox.query().build().find(recommendConfigDb.offset, recommendConfigDb.limit)
    ...
    override suspend fun saveListMovie(listDbMovie: List<DbMovie>) {
        val listDbMovieDistinct =
            listDbMovie.filter { movieNetwork ->
